...
def query_production_cell_candidates(
    ...
    probe = (
        ...
        "local enabled_items={};"
        "for _,rr in pairs(game.forces.player.recipes) do "
        "if rr.enabled then for _,p in ipairs(rr.products) do "
        "if p.type=='item' then enabled_items[p.name]=true end end end end;"
        "local unlocked={};"
        "for _,e in pairs(by_name) do "
        "local it=e.items_to_place_this and e.items_to_place_this[1];"
        "if (not it) or enabled_items[it] then unlocked[e.name]=e end end;"
        "if next(unlocked)~=nil then by_name=unlocked end;"
        "local out={};"
        "for _,e in pairs(by_name) do "
...
def place_production_cell(client, surface, item, hint="", requesting_player="", knobs=None):
    ...
    phase1 = (
        ...
        "local enabled_items={};"
        "for _,r in pairs(game.forces.player.recipes) do "
        "if r.enabled then for _,p in ipairs(r.products) do "
        "if p.type=='item' then enabled_items[p.name]=true end end end end;"
        "local unlocked={};"
        "for _,e in ipairs(candidates) do "
        "local item=e.items_to_place_this and e.items_to_place_this[1];"
        "if (not item) or enabled_items[item] then unlocked[#unlocked+1]=e end end;"
        "if #unlocked>0 then candidates=unlocked end;"
...
def build_lookup_prompt(question, slices):
    ...
    return (
        ...
        "- If the question asks WHERE an item is, or where a stash is, list "
        "the matching container positions as [gps=x,y,surface] in the result "
        "(math.floor the coordinates) with each one's count, not just a total; "
        "only the container(s) actually holding the named item qualify.\n"
...
def build_classification_prompt(
    ...
    return (
        ...
        "and include every requested item. Use LOGISTICS ONLY for requests about "
        "what is available, on hand, or in stock in the player's LOGISTIC "
        "NETWORK (bot-provided stock) on a named planet. 'In storage', 'stash', "
        "'where is X kept/stored', 'in a chest/container/box', or any question "
        "about a physical location is NOT the logistic network — it means items "
        "physically sitting in containers on a surface, so use LOOKUP. When a "
        "player asks WHERE something is stored, they want container locations, "
        "not a network count. Use surface 'all' when asked about anywhere, "
        "everywhere, all planets, or the whole solar system. Examples: "
        ...
        "surface, wants a grand total of an item that exists, OR asks where an "
        "item is stored or kept physically, use LOOKUP — it composes an "
        "inventory-aware scan and reports exactly what it scanned, including "
        "container positions when the question asks for a location.\n"
